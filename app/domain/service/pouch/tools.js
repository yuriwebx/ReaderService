define([
   ...
   // HOTFIX for Promise
    if(!Promise || true){           // jshint ignore:line
        var Promise = function(fn){ // jshint ignore:line
            return $q(fn);
        };
    ...
    // HOTFIX for Set (strings only)
    // MySet becuse in comparing with Set
   var MySet = function (array) {
      ...
      // This function is not present in original Set
      function toArray() {
   ...
   // function __limit_data(limit, offset){
   //    offset = offset*1 || 0;
   //    return limit>0 ? function(data){
   //       return data.slice(offset, limit);
   //    } : offset>0 ? function(data){
   ...
   // try to use filterData() as few as possible!
      ...
      return function (data) {
         ...
         return data.filter(function (item) {
            for (var selector in filter) {
               if (filter[selector] && getValue(item, selector) !== filter[selector]) {
                  return false;
               }
            }
            return true;
   ...
   // find value by point-divided selector like 'book.meta.author'
   function getValue(obj, selector) {
      var selectors = (selector || "").split('.').filter(function (item) {
         return item !== '';
      });
      var result = obj;
      while (selectors.length > 0) {
         result = result[selectors.shift()];
         if (!result) {
            break;
         }
      }
   ...
   function pouchDefault(defaultValue){
   ...
   /////////////////////////////////////////////////////////////////////////////////////////////////////////
   // THIS IS A COPY OF SERVER-SIDE FILE
 ...
 * @param {String} path
 */
function conflictResolveError(originalValue, patchValue, path){   // jshint ignore:line
...
function isObject(value) {
   ...
   return {
      ...
      Promise : Promise, // jshint ignore:line
      MySet : MySet,
      crc32 : crc32,
      clone : clone,
